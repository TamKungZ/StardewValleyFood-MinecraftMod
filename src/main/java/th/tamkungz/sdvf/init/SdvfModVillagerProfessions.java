...
public class SdvfModVillagerProfessions {
	...
	public static final DeferredRegister<VillagerProfession> PROFESSIONS = DeferredRegister.create(ForgeRegistries.VILLAGER_PROFESSIONS, SdvfMod.MODID);
	public static final RegistryObject<VillagerProfession> FOOD_VENDOR = registerProfession("food_vendor", () -> Blocks.FURNACE, () -> ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.fire.ambient")));
	...
	private static RegistryObject<VillagerProfession> registerProfession(String name, Supplier<Block> block, Supplier<SoundEvent> soundEvent) {
		...
		return PROFESSIONS.register(name, () -> {
			...
			return new VillagerProfession(SdvfMod.MODID + ":" + name, poiPredicate, poiPredicate, ImmutableSet.of(), ImmutableSet.of(), soundEvent.get());
		});
	}
		...
		event.register(ForgeRegistries.Keys.POI_TYPES, registerHelper -> {
			for (Map.Entry<String, ProfessionPoiType> entry : POI_TYPES.entrySet()) {
				...
				if (existingCheck.isPresent()) {
					SdvfMod.LOGGER.error("Skipping villager profession " + name + " that uses POI block " + block + " that is already in use by " + existingCheck);
					continue;
				}
